...
const summaryTimeMachine7DaysGett = async (req: NextApiRequest, res: NextApiResponse) => {
    const { date, candidateId } = req.query
    const listData = await client.dataContentByTime.findMany({
        where: {
            candidateId: Number(candidateId),
            date: {
                gte: new Date(moment(new Date()).subtract(7, 'days').format("YYYY-MM-DD")),
...
export default summaryTimeMachine7DaysGett
